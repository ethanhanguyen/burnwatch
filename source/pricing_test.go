...
func TestCostForModel(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, approx := CostForModel(tt.model, tt.input, tt.output, tt.cacheRead, tt.cacheWrite)
...
func TestCostForModelNonNegative(t *testing.T) {
	got, _ := CostForModel("any-model", -5, -10, -3, -1)
	if got < 0 {
...
func TestCostForModel_FetchedPricingMatch(t *testing.T) {
	...
	got, approx := CostForModel("vercel/deepseek/deepseek-v4-pro", 1000, 1000, 0, 0)
...
func TestCostForModel_SubstringMatchLongestKey(t *testing.T) {
	...
	got, approx := CostForModel("claude-sonnet-4-5-20250929", 1000, 0, 0, 0)
...
func TestCostForModel_EmptyFetchedPricing(t *testing.T) {
	...
	got, approx := CostForModel("deepseek-v4-pro", 1000, 0, 0, 0)
	if !approx {
		t.Error("expected approximate for model not in embedded pricing")
	}
	// Falls back to sonnet-tier fallback since "deepseek" not in embedded table
	_ = got
}
...
func TestCostForModel_ExactMatchPrecedence(t *testing.T) {
	...
	got, approx := CostForModel("claude-sonnet-4-5", 1000, 0, 0, 0)
...
func TestLookupPrice_NoFetchedNoEmbedded(t *testing.T) {
	...
	p, approx := lookupPrice("unknown-model-xyz")
	if !approx {
		t.Error("expected approximate for unknown model")
	}
	if p.input != fallback.input || p.output != fallback.output {
		t.Errorf("expected fallback pricing, got %+v", p)
	}
}
...
func TestCostForModel_DeepseekMatched(t *testing.T) {
	...
	got, approx := CostForModel("deepseek/deepseek-v4-pro", 1000, 500, 0, 0)
	wantInput := 0.435
	wantOutput := 0.87 * 0.5
