...
func generateSyntheticEvents(n int) []source.TokenEvent {
	...
	for i := 0; i < n; i++ {
		...
		cost := source.CostForModel(model, in, out, cr, cw)
...
func BenchmarkPipeline_1K(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		_ = analyze.DetectWaste(events, baselines, cfg.Thresholds.CostOutlierSigma, allToggles)
	}
}
...
func BenchmarkWasteDetection(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = analyze.DetectWaste(events, baselines, 2.0, allToggles)
	}
}
...
func BenchmarkPricingLookup(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		ti := i % len(tokens)
		tok := tokens[ti]
		_ = source.CostForModel(models[mi], tok.in, tok.out, tok.cr, tok.cw)
	}
}
