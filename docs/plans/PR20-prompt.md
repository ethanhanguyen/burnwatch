# PR20: ML Pipeline — Supervised Waste Classification (Experimental)
...
- **Must merge first:** PR19 (LLM verification — generates initial labeled data via LLM verdicts)
- **External dependencies:** None
- **Can be parallel with:** None (serial after PR19)
...
- [ ] Create feature branch: `git checkout -b pr20-ml-pipeline`
...
```
Type: FeatureVector = [10]float64  (same features as PR18 anomaly detection)
...
- [ ] Pull latest main
- [ ] Create feature branch from main: `git checkout -b pr20-ml-pipeline`
...
- [ ] Push to branch `pr20-ml-pipeline`
- [ ] Open pull request
...
- Features are identical to PR18 (Isolation Forest). Shared feature extraction function in `analyze/anomaly.go` — refactor to `analyze/features.go` that both use.
