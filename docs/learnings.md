...
- **Embedded pricing units must match fetched pricing format.** Fetched OpenRouter pricing converts per-token API prices to $/1K by multiplying by 1000. The embedded `pricing` table must also use $/1K values, not $/MTok. Using $/MTok values (e.g. `{3.00, 15.00}`) in a $/1K calculation path inflates costs 1000x. `source/pricing.go:12-28,90-95`
